...
class TestWsgiApplication(WsgiTestBase):
    ...
    def test_wsgi_metrics_no_labeler_no_leak(self):
        app_custom = otel_wsgi.OpenTelemetryMiddleware(
            error_wsgi_unhandled_custom_attrs
        )
        custom_environ = self.environ.copy()
        self.assertRaises(
            ValueError, app_custom, custom_environ, self.start_response
        )

        app_no_labeler = otel_wsgi.OpenTelemetryMiddleware(simple_wsgi)
        no_labeler_environ = self.environ.copy()
        response = app_no_labeler(no_labeler_environ, self.start_response)
        for _ in response:
            pass
